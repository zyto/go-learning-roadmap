...
## 3. Как создать структуру через литерал
...
## 7. Функция тоже получает копию структуры
...
## 9. Если указатель может быть `nil`
...
## 17. DTO — простая модель передачи данных
...
Проверь группы `devices` и `furniture`.
...
Проверь валидный и невалидный пример.
...
* `findProductByID` ищет товар по `ID`
* `activeProducts` возвращает новый список активных товаров
* `totalPrice` считает сумму цен
* `groupProductsByCategory` группирует товары по категории
* `deactivateProduct` меняет `Active` на `false` у найденного товара в исходном `slice`
